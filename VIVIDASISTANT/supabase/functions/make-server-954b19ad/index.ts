 ...
 * Klient posílá Bearer anon + volitelně `X-User-Access-Token` s access_token uživatele.
 */
...
app.post("/make-server-954b19ad/crm/org-detail", async (c) => {
  try {
    ...
    const activitiesResp = await fetch(`${PIPEDRIVE_BASE}/organizations/${org.id}/activities?limit=20&api_token=${pipedriveToken}`);
