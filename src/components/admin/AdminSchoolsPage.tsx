...
export function AdminSchoolsPage() {
  ...
  const allShopOrders = detail?.orderSummary.recentOrders ?? [];
  const ORDERS_PREVIEW = 8;
  const visibleShopOrders =
    ordersExpanded || allShopOrders.length <= ORDERS_PREVIEW
      ? allShopOrders
      : allShopOrders.slice(0, ORDERS_PREVIEW);
                  ...
                  {allShopOrders.length > 0 ? (
                    <div className="mt-4 space-y-2">
                      <p style={FF} className="text-[11px] text-[#001161]/45 mb-2">
                        {`Celkem ${allShopOrders.length} objednávek v e-shopu (podle IČO / názvu).`}
                      </p>
                      ...
                      {allShopOrders.length > ORDERS_PREVIEW ? (
                        <button
                          type="button"
                        ...
                        >
                          {ordersExpanded
                            ? 'Sbalit seznam'
                            : `Zobrazit všech ${allShopOrders.length} objednávek`}
                        </button>
                      ) : null}
                    </div>
                  ) : (
