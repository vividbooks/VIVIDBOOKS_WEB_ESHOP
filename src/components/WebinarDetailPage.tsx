...
export function WebinarDetailPage({ webinar }: WebinarDetailPageProps) {
  ...
  /** E-mail/jméno přímo v dotazníku, když není vyžadována registrace (`surveyRequireFullRegistration === false`). */
  const [surveyOpenEmail, setSurveyOpenEmail] = useState('');
  const [surveyOpenName, setSurveyOpenName] = useState('');
  ...
  useEffect(() => {
    ...
    if (isDvpp) {
      ...
      setSurveyOpenEmail(em);
      setSurveyDeepLink(true);
      if (requireFullSurveyReg) setSurveyRegOk(null);
  ...
  }, [webinar.id, webinar.isPast, postSurveyMerged.length, preSurveyQuestions.length, requireFullSurveyReg]);
  ...
  useEffect(() => {
    if (!isSurveyFullPage || !requireFullSurveyReg) return;
  ...
  }, [isSurveyFullPage, requireFullSurveyReg, webinar.id, form.email]);
    ...
    return (
      ...
      >
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
          <WebinarPostSurvey
            webinar={webinar}
            email={requireFullSurveyReg ? form.email : surveyOpenEmail}
            participantName={requireFullSurveyReg ? form.name : surveyOpenName}
            surveyIdentityInline={!requireFullSurveyReg}
            onSurveyIdentityEmailChange={setSurveyOpenEmail}
            onSurveyIdentityNameChange={setSurveyOpenName}
