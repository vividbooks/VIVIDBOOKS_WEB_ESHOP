...
/** Cena z katalogu je autoritativní (server), textová cena z formuláře je jen fallback
 *  pro položku, kterou v katalogu nenajdeme. */
function unitPriceHalerFromItemOrCatalog(itemPrice: unknown, catalogItem: unknown): number {
  const fromCatalog = getProductUnitPriceInHaler(catalogItem);
  if (fromCatalog > 0) return fromCatalog;
  const fromItem = parsePriceTextToKc(itemPrice);
  return fromItem !== null ? Math.round(fromItem * 100) : 0;
}
