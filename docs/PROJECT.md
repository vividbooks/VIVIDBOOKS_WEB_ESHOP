...
- **Base.com / iDoklad:** při odeslání formuláře nic (deal v Pipedrive vzniká jako `open`). Až obchodník deal označí **won**, `pipedrive-inbound-deal` přepíše položky, nastaví dopravu **PPL** + platbu převodem (formulář má `shipping_method='none'` / `payment_method='invoice'`, což by v Base vyšlo jako „none“ / „Platba kartou“), **doručovací adresu vezme z PD organizace** (ne ze sídla ARES uloženého při odeslání formuláře — IČO u OSVČ často míří na bydliště, zatímco v CRM je výdejna) a zařadí **Base.com** stejně jako u ručního CRM dealu. iDoklad se neposílá. `cancel-stale-orders` i `payment-reminders` distributorské objednávky pořád ignorují (čekají na won v PD).
