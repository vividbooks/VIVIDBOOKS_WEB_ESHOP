...
- **Base.com při výhře dealu:** vyhraný deal v pipeline 8 je pokyn k expedici — `pipedrive-inbound-deal` přepíše položky podle dealu a **zařadí Base export** (`addOrder`, u už exportované objednávky `setOrderStatus`). Zároveň srovná řádek `orders` do podoby, kterou Base čeká: adresa z Pipedrive (když je kompletní — jinak zůstane ta z formuláře / ARES), doprava `ppl` + `PIPEDRIVE_INBOUND_SHIPPING_PRICE_HALER` (default 89 Kč), platba `transfer` a `pending_payment`/`incomplete` → `processing`. V `order_events` to poznáte podle `distributorBaseExport: true`.
- **iDoklad:** nic — fakturuje obchod mimo e‑shopovou frontu (`pipedrive-inbound-deal` zařazuje jen službu `basecom`). `cancel-stale-orders` i `payment-reminders` distributorské objednávky ignorují.
