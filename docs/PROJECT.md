...
| `PIPEDRIVE_TRIAL_NATIVE_LABEL_NAME`, `PIPEDRIVE_TRIAL_CASE_FIELD_NAME`, `PIPEDRIVE_TRIAL_CASE_OPTION_NAME` | Názvy (ne ID) toho, na čem stojí podmínky automatizace **Trial CTA 01** — nativní štítek dealu „Trial web (interactive)" a `Case = New`. Hledá se podle názvu schválně: kdyby se ID změnilo, obchod by podmínkou tiše neprošel a učiteli by nedorazily kódy. Pozor na dvojici podobných jmen — `Trial web (interactive)` je **nativní štítek**, kdežto `Trial web (interactive) - 2.0` je volba 359 vlastního pole 12463, kterou si e‑shop zavedl pro chybové větve. |
