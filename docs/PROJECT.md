...
| `PIPEDRIVE_PERSON_POSITION_FIELD_KEY`, `PIPEDRIVE_PERSON_SUBJECT_FIELD_KEY`, `PIPEDRIVE_PERSON_STAGE_FIELD_KEY` | Volitelné override hash klíčů vlastních polí **osoby** pro trial/webinář: pozice (UI ID 9093), předmět (9095), stupeň (9099). Bez nastavení se klíče auto-detekují přes `/personFields` (pozice navíc přes známý default hash). Z trial formuláře (`/vyzkousejte`) se mapuje: pozice z `<select>` (Učitel→164, Ředitel→159, Zástupce→160, Metodik→274, Rodič→166, Jiné→172), předmět z výběru (Fyzika→309, Chemie→310, Matematika→311, Přírodopis→312, Prvouka→413, Český jazyk→414, Jiné→319) a stupeň (1. stupeň→421, 2. stupeň→422). Pole 9095/9099 typu `enum` dostanou jednu hodnotu, `set` všechny vybrané. U předmětu (9095) je výběr z formuláře **autoritativní** (`supabase/functions/_shared/pipedrive-person-subject.ts`): „Jiné" (319), které tam zapíše legacy `api.vividbooks` při zakládání osoby, se odebere / přepíše, jakmile učitel označil konkrétní předmět; ručně vybraný konkrétní předmět z CRM se nepřepisuje. **Registrace na webinář** se na předmět / stupeň ptá stejnými volbami jako trial formulář (`src/utils/trialSubjectOptions.ts`, komponenta `WebinarRegistrationFormFields`); pozice určuje otázku: učitelské a pedagogické pozice vybírají předměty (stupeň si server odvodí), ředitel / zástupce / poradce / metodik vybírá stupeň, rodič a „Jiné“ nevybírají nic (a „Nejsem pedagog“ se neptáme vůbec). `POST /webinar-registrace` výběr ukládá do KV registrace (vidí ho i admin a CSV export) a `syncWebinarRegistrationToPipedrive` ho rovnou zapisuje do polí osoby 9095 / 9099 — stejnými buildery jako trial, takže „Other“ ustoupí konkrétnímu předmětu a stupeň se sjednocuje. Trial založený hned po registraci (`WebinarPostRegistrationTrial`) posílá týž výběr dál do legacy API — a nabízí se **jen tomu, kdo u „Používám Vividbooks“ odpověděl Ne** (odpověď z dotazníku, jinak z registračního formuláře). Stávajícím uživatelům legacy API vrací „Email is used yet.“ místo kódů, takže by po nich v CRM zůstal jen deal s labelem Trial 2.0. Dřív se odtud posílal natvrdo `Other-2` (→ předmět „Other“ 319) a později natvrdo `SchoolStage-2`, takže 9095 / 9099 byly u trialů z webináře prázdné nebo špatně. |
| `PIPEDRIVE_ESHOP_*` | E‑shop dealy v Pipedrivu (viz `.env.example`). |
| `TRIAL_CREATED_PIPEDRIVE_ENABLED` | Zapíná zakládání **obchodu k novému webovému trialu** (`POST /trial-created-pipedrive`, scénář `trial_created`). Výchozí stav **vypnuto**: dokud běží scénář Make „[CZ1] Trial form", zakládá obchod on a druhý obchod by znamenal dvakrát odeslané kódy. Přepnout ve chvíli, kdy se scénář v Make vypne. |
| `PIPEDRIVE_TRIAL_NATIVE_LABEL_NAME`, `PIPEDRIVE_TRIAL_CASE_FIELD_NAME`, `PIPEDRIVE_TRIAL_CASE_OPTION_NAME` | Názvy (ne ID) toho, na čem stojí podmínky automatizace **Trial CTA 01** — nativní štítek dealu „Trial web (interactive)" a `Case = New`. Hledá se podle názvu schválně: kdyby se ID změnilo, obchod by podmínkou tiše neprošel a učiteli by nedorazily kódy. Pozor na dvě podobné volby **téhož** štítku (pole 12463 je nativní `label` dealu): volbu `Trial web (interactive)` (52) nese obchod od Make a na ni se automatizace chytá, kdežto `Trial web (interactive) - 2.0.` (359) si e‑shop zavedl pro chybové větve, kde e‑mail zákazníkovi chodit nemá. Happy path proto 359 nenastavuje. |
| `PIPEDRIVE_OWNER_REDIRECTS` | Mapa `zdroj:cíl` (výchozí `11629944:18026774`) — cokoli by připadlo Danielu Ondráškovi, jde na Gabrielu Švédovou. Aplikuje se jako poslední krok nad výsledkem celého rozhodování o vlastníkovi, včetně explicitního overrideu. Neřetězí se. |
| _(bez ENV)_ `_shared/pipedrive-region-owner.ts` | Kraj → obchodník, převzato z Make datastore 17499. Nový mezikrok v `resolvePipedriveDealOwnerUserId` mezi org polem `current_deal_owner` a vlastníkem existujících dealů. Kraj se bere ze `skoly.csv` podle IČO, jinak z adresy organizace. **Vlastník dealu určuje, ze které schránky odejde CTA 01** — každý obchodník má vlastní kopii té automatizace nastavenou na sebe. |
...
| Proměnná | Účel |
|----------|------|
| `FULFILMENT_STOCK_URL` | **Povinná pro zapnutí.** Feed nebo REST endpoint se stavem skladu. Formát se detekuje sám: JSON, XML nebo CSV. |
| `FULFILMENT_STOCK_TOKEN` | Volitelné. Posílá se jako `Authorization: Bearer …`. |
| `FULFILMENT_STOCK_TOKEN_HEADER` | Volitelné — jiná hlavička pro token (např. `X-Api-Key`). |
| `FULFILMENT_STOCK_SKU_FIELDS` | Volitelné, čárkou oddělené názvy polí s kódem. Default zkouší `sku`, `code`, `kód`, `ean`, … |
| `FULFILMENT_STOCK_QUANTITY_FIELDS` | Volitelné, pole s počtem. Default `quantity`, `stock`, `množství`, … |
...
- **Formulář:** IČO, e‑mail, telefon, počty kusů (ceny se nezobrazují) a poznámka. Žádná adresa, doprava ani platba.
- **Endpoint:** `POST …/make-server-93a20b6f/distributor/orders` (token v těle / hlavička `X-Distributor-Token`) — založí řádek v `public.orders` (`source='distributor'`, `status='pending_payment'`, `payment_method='invoice'`, `shipping_method='none'`, `customer_email` + `customer_phone` z formuláře) + `order_items` s cenami z katalogu. Název a sídlo firmy dohledá podle IČO (CSV škol → ARES). Idempotence přes `idempotency_key = distributor:<submissionId>`.
- **E‑mail:** po založení objednávky `send-order-email` s typem `distributor_order_received` na zadaný kontaktní e‑mail — shrnutí (firma, IČO, kontakt, položky bez cen, poznámka). Selhání Mandrillu neblokuje odpověď.
- **Pipedrive:** `syncEshopOrderToPipedriveFromDb(orderId, 'distributor_open')` → pipeline **8**, fáze **43**, organizace **jen podle IČO** (pole CIN, `strictIcoMatch`; když IČO v CRM není, organizace se založí), kontaktní osoba se **dohledá podle e‑mailu z formuláře** (`/persons/search` + výpis nedávných + osoby u org); **když v CRM není, založí se nová** (jméno: OSVČ z IČO, jinak lokální část e‑mailu — ne název s.r.o.) a přiřadí k dealu; když chybí e‑mail, deal jde jen s organizací, produkty z katalogu, HTML note k dealu s **e‑mailem, telefonem a poznámkou**, vyplněné pole „Eshop ID“. Bez štítků B2C/B2B a bez „Source = PRINT“.
- **Sleva na řádcích dealu:** každý produkt dostane `discount` + `discount_type='percentage'` — **25 %**, u IČO `06745342`, `49709895`, `73565211` **35 %** (porovnává se bez úvodních nul; přepis přes `PIPEDRIVE_DISTRIBUTOR_DISCOUNT_*`). Ceny v `orders` zůstávají ceníkové, sleva žije jen na dealu. E‑shopové dealy slevu nedostávají.
- **Base.com při won:** objednávka čeká v `pending_payment`, dokud obchodník deal v pipeline 8 nevyhraje. Won webhook (`pipedrive-inbound-deal`, scénář B) přepíše položky podle dealu, posune objednávku do `processing`, dorovná dopravu na **PPL 89 Kč** a platbu na **převod** (z `none`/0/`invoice`, jinak by do Base šlo `delivery_method: "none"`) a zařadí **stejný Base export** jako u eshopových objednávek (`addOrder`, resp. `setOrderStatus` když už `basecom_order_id` existuje). V `order_events` to poznáš podle `distributorBaseExport: true`.
- **iDoklad:** nic — scénář B fakturu nefronťuje vůbec, fakturaci distributorům řeší obchod mimo e‑shop. `cancel-stale-orders` i `payment-reminders` distributorské objednávky ignorují.
